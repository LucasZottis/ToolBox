...
namespace BibliotecaPublica.CaixaFerramenta.Componentes.Dados
{
    ...
    public class NavegadorDados : BindingNavigator, ISupportInitialize
    {
        #region Delegados
        ...
        public FonteDados FonteDados
        {
            ...
            {
                _fonteDados = value;
                SetarFonteLigacao( ref _fonteDados, value );
            }
        }
        ...
        public NavegadorDados() : this( false )
        {

        }

        public NavegadorDados( FonteDados fontaDados ) : this( true )
        {
            FonteDados = fontaDados;
        }

        [EditorBrowsable( EditorBrowsableState.Never )]
        public NavegadorDados( IContainer container ) : this( false )
        {
            if ( container == null )
            {
        ...
        public NavegadorDados( bool adicionarItensPadroes )
        {
        ...
        private void AoVoltarParaPrimeiroItem( object enviador, EventArgs argumento )
        {
            if ( Validate() )
            {
                if ( _fonteDados != null && _fonteDados.DataSource != null )
                {
                    _fonteDados.MoveFirst();
                    RefreshItemsCore();
                }
            }
        }
        ...
        private void AoClicarIncluirItem( object enviador, EventArgs argumento )
        {
            ...
            {
                _fonteDados.AdicionarNovo();
            }

            _fonteDados.MoveLast();

            _linha = _fonteDados.ItemSelecionado;
            _linha?.BeginEdit();
        ...
        private void AoClicarDeletarItem( object enviador, EventArgs argumento )
        {
            ...
            if ( _fonteDados.ItemSelecionado == null )
            {
            ...
            if ( _aoDeletarItem != null )
            {
                try
                {
                    _aoDeletarItem( _fonteDados.ItemSelecionado[ _fonteDados.NomeValorChave ].ParaInt() );
                }
        ...
        private void AoClicarGravar( object enviador, EventArgs argumento )
        {
            ...
            try
            {
                ...
                if ( !Validate() )
                {
                    _fonteDados.ItemSelecionado.CancelEdit();
                    return;
                }
                ...
                {
                    _fonteDados.ItemSelecionado.EndEdit();
                }
        ...
        private void SetarFonteLigacao( ref FonteDados antigaFonteLigacao, FonteDados novaFonteLigacao )
        {
        ...
        private bool Pesquisar()
        {
            ...
            if ( _pesquisando && !( Disposing || IsDisposed ) )
            {
                ...
                if ( _fonteDados != null && codigo.Diferente( 0 ) )
                {
                    posicao = _fonteDados.Encontrar( codigo );
                }
        ...
        protected override void RefreshItemsCore()
        {
            ...
            if ( !DesignMode )
            {
                ...
                {
                    permitirIncluirNovoItem = ( _fonteDados as IBindingList ).AllowNew;
                    permitirEditar = ( _fonteDados as IBindingList ).AllowEdit;
                    permitirExcluir = ( _fonteDados as IBindingList ).AllowRemove;
                    permitirGravar = ( ( _fonteDados as IBindingList ).AllowNew || ( _fonteDados as IBindingList ).AllowEdit );
                    permitirCancelar = ( permitirGravar && ( _fonteDados as IBindingList ).AllowNew || ( _fonteDados as IBindingList ).AllowEdit );
                    permitirPesquisar = _formularioInterface.ModoJanela == ModoJanela.Navegacao && _fonteDados.Count > 1;
                    permitirNavegar = _formularioInterface.ModoJanela == ModoJanela.Navegacao && _fonteDados.Count > 1;
                }
            }
